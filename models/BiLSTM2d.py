...
class BiLSTM(nn.Module):
    def __init__(self, batch_size=64, in_channel=1, out_channel=10):
        ...
        self.hidden2label1 = nn.Linear(self.hidden_dim * 2 * self.V*self.V, self.hidden_dim * 2)
        self.hidden2label2 = nn.Linear(self.hidden_dim * 2, C)
        self.hidden = self.init_hidden(self.num_layers, batch_size)
        self.bilstm = nn.LSTM(16, self.hidden_dim, num_layers=1, bidirectional=True,
                              bias=False)
    ...
    def forward(self, x): # BS*1*-1
        ...
        x = x.view(-1,16,self.V*self.V)
        x = torch.transpose(x, 1, 2)
        x = torch.transpose(x, 0, 1)  # V*BS*16
        x = self.dropout(x)
        bilstm_out,_ = self.bilstm(x) # V*BS*2H
